...
void win_init(win_t *win)
{
	...
	win->light = win_luminance(&(win->win_bg)) > win_luminance(&(win->win_fg));

#if HAVE_LIBFONTS
	// ;madhu 210923 - bar foreground == win background and vice versa
	bar_bg = win_res(db, RES_CLASS ".bar.background", DEFAULT_BAR_BG ? DEFAULT_BAR_BG : win_fg);
	bar_fg = win_res(db, RES_CLASS ".bar.foreground", DEFAULT_BAR_FG ? DEFAULT_BAR_FG : win_bg);
